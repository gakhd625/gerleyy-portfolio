      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-y-10 gap-x-12">
        {skillCategories.map((category, index) => (
          <div key={index} className="flex items-start gap-4">
            <div className="w-11 h-11 rounded-full bg-gray-100 dark:bg-slate-800 flex items-center justify-center text-lg flex-shrink-0">
              <span aria-hidden="true">{category.icon}</span>
            </div>
            <div className="min-w-0 space-y-1">
              <h3 className="text-base font-semibold text-black dark:text-white">{category.title}</h3>
