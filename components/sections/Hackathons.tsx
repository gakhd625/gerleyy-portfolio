  ...
  return (
    <section id="hackathons" className="mb-24 scroll-mt-8">
      <div className="inline-block px-3 py-1 bg-white dark:bg-slate-800 border border-black dark:border-gray-700 rounded mb-4">
        <span className="text-sm font-medium text-black dark:text-white">Hackathons</span>
      </div>
      <h2 className="text-3xl font-bold mb-4 text-black dark:text-white">I like building things.</h2>
      ...
      <div className="space-y-8 relative">
        ...
        {portfolioData.hackathons.map((hackathon, index) => (
          <div key={index} className="relative flex gap-6 pl-12">
            <div className="absolute left-0 w-12 h-12 rounded-full bg-gray-200 dark:bg-slate-700 flex items-center justify-center flex-shrink-0 text-xl z-10 overflow-hidden">
            ...
            <div className="flex-1">
              <p className="text-sm text-gray-500 dark:text-gray-500 mb-1">{hackathon.date}</p>
              <h3 className="text-xl font-semibold mb-2 text-black dark:text-white">{hackathon.title}</h3>
              <p className="text-sm text-gray-500 dark:text-gray-500 mb-2">{hackathon.location}</p>
              ...
              {hackathon.award && (
                <div className="flex items-center gap-2 text-sm">
                  <span>🏆</span>
                  <span className="text-gray-700 dark:text-gray-300">{hackathon.award}</span>
                </div>
              )}
            </div>
          </div>
