...
export default function Hackathons() {
  return (
    <section id="hackathons" className="mb-24 scroll-mt-8">
      ...
      <h2 className="text-3xl font-bold mb-4">I like building things.</h2>
      ...
      <div className="space-y-8 relative">
        ...
        {portfolioData.hackathons.map((hackathon, index) => (
          <div key={index} className="relative flex gap-6 pl-12">
            <div className="absolute left-0 w-12 h-12 rounded-full bg-gray-800 flex items-center justify-center flex-shrink-0 text-xl z-10">
              {hackathon.logo}
            </div>
            <div className="flex-1">
              ...
              {hackathon.award && (
                <div className="flex items-center gap-2 text-sm">
                  <span>🏆</span>
                  <span className="text-gray-300">{hackathon.award}</span>
                </div>
              )}
            </div>
          </div>
